...
updated: 2026-08-31T00:30:00.000Z
---
...
reason: "The deployed Preview has 43 visible source links, a visible verification notice, four labeled controls, no stale-link 404/certificate failure, ordered focusable controls, and no horizontal overflow at the tested viewport. The Preview-only accessibility fixture exposes the formerly unobservable zero-result directory, zero-programme locker, and long-Unicode states; automated browser verification confirmed its noindex/nofollow metadata, zero links/buttons/inputs, required headings, and no horizontal overflow. Focused Phase 5 regressions pass 42/42, including rate-limit, campus-community, release accessibility, and fixture suites. Human evidence received: on the authorized shareable Preview fixture, a tester reported that NVDA announced the fixture headings in logical order, including the zero-result heading and 'Program details before you apply'; at 200% Firefox zoom, the tester reported no cutoff, overlap, or horizontal left-right scrolling, while the page became taller as expected. The voice transcript did not capture the exact UTC time or NVDA/Firefox versions. The tester then started at Control+Home and used heading navigation; NVDA announced the long-Unicode `École supérieure d'études technologiques…` content as a level-two heading clearly enough for the tester to identify it. This is a passed human long-Unicode-heading observation. The tester has also reported that, on `/schools/north-valley-college`, they activated `Report this note`, chose Cancel without confirming, focus returned to that same `Report this note` button, and no error page occurred. This is a passed human accessibility observation for the report-dialog cancel path; it supersedes the earlier failed/indeterminate 404 report. On `/schools/metro-technical-institute`, the tester tabbed to `Sign in to leave a note, link`, activated it, and reached the sign-in page. This is a passed human empty-state recovery/navigation observation. On `/western-new-york`, NVDA announced the level-one main heading `Compare access, admissions and campus environment sources before you apply`, introductory source-directory content, and a populated Northland Workforce Training Center result. NVDA form navigation reached the clearly labelled Transportation, Standardized tests, and GPA record combo boxes and the unchecked Child-care-aware planning checkbox; link navigation announced `Official site, link`. This is a passed human directory accessibility observation. On the fixture, NVDA form-field navigation returned `No next form field` and button navigation returned `No next button`; the previously encountered Vercel toolbar was identified as Preview-host chrome and is excluded from the fixture result. A first automated snapshot was taken before the asynchronous public-note load completed and appeared empty; a later stable authenticated Preview session listed six generated UAT notes. The only remaining human observation is the fixture's link/source-label reading: it must expressly confirm `No next link` and that `Official source label: Études, apprentissage, 漢字, العربية — verification example only` is announced coherently. Test 1 remains blocked pending that explicit fixture observation; no unreported result is inferred."
