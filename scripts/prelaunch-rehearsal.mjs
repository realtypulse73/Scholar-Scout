...
export function aggregateReleaseRecords(records, candidateCommit) {
  if (!REQUIRED_RELEASE_LANES.every((lane) => validateReleaseRecord(records[lane], lane, candidateCommit))) {
    return false;
  }
...
function areDistinctPreviewRecordTargets(records) {
  const baselineTarget = records['preview-browser']?.target;
  const outageTarget = records['preview-outage']?.target;
  return isPreviewDeploymentUrl(baselineTarget) && isPreviewDeploymentUrl(outageTarget)
    && baselineTarget !== outageTarget;
}

function isPreviewDeploymentUrl(value) {
