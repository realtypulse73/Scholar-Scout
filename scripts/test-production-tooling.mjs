...
test('prelaunch workflow orders candidate proof before independent Preview lanes and aggregation', async () => {
  ...
  assert.doesNotMatch(workflow, /Install Vercel deployment reader/);
  ...
  assert.doesNotMatch(workflow, /SCHOLARSCOUT_VERCEL_(BASELINE|OUTAGE)_TOKEN/);
  ...
  assert.doesNotMatch(workflow, /--vercel-scope scholar-scout/);
  assert.doesNotMatch(workflow, /SCHOLARSCOUT_GITHUB_DEPLOYMENTS_TOKEN/);
