...
export default async function SchoolLockerPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-ink-50 text-ink-900">
      <nav className="mx-auto flex max-w-6xl flex-col items-start gap-3 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2"><Link href="/peer-community" className="text-sm font-semibold text-ink-600">Campus conversations</Link><AuthStatusLink /></div>
      </nav>
