...
export default function UploaderContactPanel({ username, programId, inboxEnabled }: UploaderContactPanelProps) {
  ...
  return (
    <div className="mt-3 rounded-card border border-border bg-silver p-4" aria-busy={isSending}>
      ...
      <textarea id={`inbox-${username}`} value={body} onChange={(event) => setBody(event.target.value)} maxLength={500} rows={4} placeholder="What would you like to know about the program or campus?" className="mt-2 min-h-touch w-full break-words rounded-control border border-border bg-white p-3 text-sm text-ink-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500" aria-describedby={`inbox-guidance-${username}`} />
      ...
      {status ? <p className="mt-3 text-sm font-semibold leading-6 text-ink-700" role="status">{status}</p> : null}
    </div>
  );
}
