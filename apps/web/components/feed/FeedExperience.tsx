    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
      <section className="space-y-4" aria-label="Pathway feed">
        {visibleItems.map((item, index) => (
          <article
            key={item.id}
            className="min-h-[78vh] scroll-mt-4 border-b border-border py-4"
            onMouseEnter={() => setActiveIndex(index)}
          >
            <div className="mx-auto flex max-w-xl flex-col gap-4">
              <div className="overflow-hidden rounded-card border border-border bg-silver">
                {item.type === 'video' ? (
                  <video
                    className="aspect-[9/16] w-full bg-silver object-cover"
                    src={item.videoUrl}
                    controls
                    playsInline
                    onPlay={() => {
                  ...
                  <div className="flex aspect-[9/16] flex-col justify-center gap-5 bg-silver p-8 text-ink-900">
          ...
          <div className="mt-4 space-y-3">
            {initialRecommendations.slice(0, 3).map((recommendation) => (
              <Link
                ...
                className="block rounded-card border border-border p-3 hover:border-brand-400"
              >
