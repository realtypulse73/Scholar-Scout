  ...
  return (
    <Card className={compact ? 'p-4' : 'p-5'}>
      ...
      <h2 className="mt-3 text-xl font-extrabold text-ink-900">
        <Link href={`/programmes/${programme.id}`} className="hover:text-brand-700">
          {programme.name}
        </Link>
      </h2>
      ...
      <section className="mt-4 rounded-card bg-ink-50 p-3" aria-label="Source and verification">
      ...
      {cautions.length > 0 ? (
        <section className="mt-4 rounded-card bg-warning-50 p-3" aria-label="Verification cautions">
          <h3 className="text-sm font-extrabold text-warning-700">Worth verifying</h3>
          <ul className="mt-2 space-y-1">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        ...
        <Link href="/shortlist" className="inline-flex min-h-10 items-center rounded-card border border-ink-300 bg-white px-4 text-sm font-semibold text-ink-700 hover:border-brand-400 hover:text-brand-700">
          Compare
        </Link>
        {evidence.sourceUrl ? (
          <Link href={evidence.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center rounded-card border border-ink-300 bg-white px-4 text-sm font-semibold text-ink-700 hover:border-brand-400 hover:text-brand-700">
            Visit source
          </Link>
        ) : null}
        <Link href="/programmes?pathway=2-year-community-college" className="inline-flex min-h-10 items-center rounded-card border border-ink-300 bg-white px-4 text-sm font-semibold text-ink-700 hover:border-brand-400 hover:text-brand-700">
          Alternate pathway
        </Link>
      </div>
    </Card>
